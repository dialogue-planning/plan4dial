...
def rollout_config(configuration):
    actions = {act: {"condition": convert_to_hovor_fluents(list(get_precond_fluents(act_cfg["condition"]))), "effect": {}} for act, act_cfg in configuration["actions"].items()}
    for act, act_cfg in configuration["actions"].items():
        for out in act_cfg["effect"]["outcomes"]:
            if "updates" in out:
                actions[act]["effect"][out["name"]] = convert_to_hovor_fluents(list(get_update_fluents(out["updates"])))
