...
def create_intent_example(
    extracted_value: str, entity: str, true_value=None
):
    if not true_value:
        ...
        if "variables" in intent_cfg:
            ...
            for variable in variables:
                ...
                if "options" in ctx_var:
                    variations[variable] = [create_intent_example(option, variable) for option in ctx_var["options"]]
                    for option, option_var in ctx_var["options"].items():    
                        variations[variable].extend(create_intent_example(v, variable, true_value=option) for v in option_var["variations"])
...
if __name__ == "__main__":
    ...
    loaded_yaml = yaml.load(
        open(str((base / "yaml_samples/advanced_custom_actions_test_v3.yaml").resolve())),
