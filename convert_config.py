...
def generate_files(yaml_filename: str, domain_name: str, output_folder: str, rbp_path: str):
    ...
    writer = open(str((Path(__file__).parent / output_folder / f"{domain_name}.json").resolve()), "w")
    ...
    domain_str, problem_str = str((Path(__file__).parent / output_folder / f"{domain_name}_domain.pddl").resolve()), str((Path(__file__).parent / output_folder / f"{domain_name}_problem.pddl").resolve())
...
if __name__ == "__main__":
    generate_files(str((Path(__file__).parent / "plan4dial" / "yaml_samples" / "advanced_custom_actions_test_v3.yaml").resolve()), "pizza", "output_files", str((Path(__file__).parent.parent / "rbp").resolve()))